...
public class SimulationRecordServiceImpl implements ISimulationRecordService{
    ...
    public ServerResponse getSimulationRecordDetail(RecordDetailRequest recordDetailRequest) throws IOException {
        switch (recordDetailRequest.getModuleName()) {
            ...
            case Const.Module.MODULE_PDOD:
                return ServerResponse.createBySuccess(getPdodSimulationRecordDetailVo(recordDetailRequest.getParametersId(),
                        recordDetailRequest.getResultId()));
    ...
    private PdodSimulationRecordDetailVo getPdodSimulationRecordDetailVo(int parametersId, int resultId) throws IOException {
        PdodSimulationRecordDetailVo pdodSimulationRecordDetailVo = new PdodSimulationRecordDetailVo();
        String uploadPath = System.getProperty("bit.gd") + File.separator + Const.UPLOAD_FILE_PATH;

        GDParameterPdod gdParameterPdod = gdParameterPdodMapper.selectByPrimaryKey(parametersId);
        FTPUtil.moveFile(Const.UPLOAD_FILE_PATH, gdParameterPdod.getTxtIxSt(), uploadPath);
        FTPUtil.moveFile(Const.UPLOAD_FILE_PATH, gdParameterPdod.getTxtIxNd(), uploadPath);

        pdodSimulationRecordDetailVo.setxLightIntensity(PdodTXTUtil
                .getNumberFromTXT(uploadPath + File.separator + gdParameterPdod.getTxtIxSt()));
        pdodSimulationRecordDetailVo.setyLightIntensity(PdodTXTUtil
                .getNumberFromTXT(uploadPath + File.separator + gdParameterPdod.getTxtIxNd()));

        pdodSimulationRecordDetailVo.setGdParameterPdod(gdParameterPdod);

        String pdodResultPath = System.getProperty("bit.gd") + File.separator + Const.RESULT_PATH_PDOD;

        GDResultPdod gdResultPdod = gdResultPdodMapper.selectByPrimaryKey(resultId);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjxxMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjxxPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjxyMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjxyPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjyxMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjyxPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjyyMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getImjyyPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejxxMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejxxPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejxyMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejxyPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejyxMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejyxPng(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejyyMat(), pdodResultPath);
        FTPUtil.moveFile(Const.RESULT_PATH_PDOD, gdResultPdod.getRejyyPng(), pdodResultPath);

        pdodSimulationRecordDetailVo.setGdResultPdod(gdResultPdod);

        pdodSimulationRecordDetailVo.setIp(PropertiesUtil.getProperty("tomcat.ip"));

        return pdodSimulationRecordDetailVo;
    }
}
